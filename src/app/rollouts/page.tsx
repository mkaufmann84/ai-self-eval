...
// Example conversations for testing rollouts
const exampleSets = {
  simple: [
    {
      id: "simple_1",
      turns: [
        { role: "user" as const, content: "Give me a list of 5 fruits" },
        { role: "assistant" as const, content: "Sure! Here's a list:", model: "manual" },
      ],
    },
  ],
  buildComputer: [
    {
      id: "build_1",
      turns: [
        { role: "user" as const, content: "In 5 steps, explain how to build a computer" },
        { role: "assistant" as const, content: "Sure!\n1. Gather the components\n2. Prepare the Case and Motherboard\n3.", model: "manual" },
      ],
    },
  ],
  story: [
    {
      id: "story_1",
      turns: [
        { role: "user" as const, content: "Write a short story about a robot" },
        { role: "assistant" as const, content: "Once upon a time, there was a robot named", model: "manual" },
      ],
    },
  ],
  code: [
    {
      id: "code_1",
      turns: [
        { role: "user" as const, content: "Write a Python function to calculate fibonacci" },
        { role: "assistant" as const, content: "Here's a fibonacci function:\n\n```python\ndef fib(n):\n    if n <= 1:\n        return n\n    return", model: "manual" },
      ],
    },
  ],
};
...
export default function ConvoTreePage() {
  ...
  const [selectionNodeId, setSelectionNodeId] = useState<string | null>(null);
  const [selectionEndIndex, setSelectionEndIndex] = useState<number>(0);
  ...
  const [selectedExample, setSelectedExample] = useState<keyof typeof exampleSets>("simple");

  const handleLoadExample = () => {
    setRuns((prev) => [...prev, ...exampleSets[selectedExample]]);
    clearPresetSelection();
  };
  ...
  const handleTextSelection = (nodeId: string, fullText: string) => {
    ...
    // Find where the selection ends in the full text
    const range = selection.getRangeAt(0);
    const container = range.commonAncestorContainer;
    const endOffset = range.endOffset;

    // Get text up to selection end
    let textUpToSelection = "";
    if (container.nodeType === Node.TEXT_NODE) {
      textUpToSelection = container.textContent?.substring(0, endOffset) || "";
    } else if (container.nodeType === Node.ELEMENT_NODE) {
      textUpToSelection = (container as Element).textContent?.substring(0, endOffset) || "";
    }

    setSelectedText(textUpToSelection);
    setSelectionNodeId(nodeId);
    setSelectionEndIndex(textUpToSelection.length);
  };
  ...
  const handleRollout = async (
    ...
    if (!selectedOption || node.role !== "assistant") {
      return;
    }

    // Build conversation history up to (but not including) this turn
    const baseTurns = collectTurnsUpToDepth(
      tree,
      selectedMap,
      node.depth,
    ...
    try {
      // Build conversation with partial assistant response + continue prompt
      const conversationMessages = buildConversationMessages(baseTurns);
      const messagesWithPartial: ChatMessage[] = [
        ...conversationMessages,
        { role: "assistant" as const, content: cutoffText },
        { role: "user" as const, content: "*(the response was interrupted. continue exactly where you left off, as if it was not interrupted)*" },
      ];

      const messages = buildCompletionMessages(messagesWithPartial);
      ...
      await Promise.all(
        safeRequests.map(async (request) => {
          ...
          if (process.env.NODE_ENV !== "production") {
            console.debug("Generating rollout", {
              chosenModel,
              temperature,
              cutoffText,
              messages,
          ...
          const tasks = Array.from({ length: count }, (_unused, index) =>
            (async () => {
              try {
                const continuation = await generateChatCompletion({
                  model: chosenModel,
                  messages,
                  temperature,
                });
                ...
                if (continuation) {
                  // Create new run with cutoff + continuation as separate turns
                  const newTurns: RunTurn[] = [
                    ...baseTurns,
                    {
                      role: node.role,
                    ...
                    {
                      role: node.role, // Consecutive assistant message
                  ...
                  if (!runs.some((run) => turnsEqual(run.turns, newTurns))) {
                    ...
                    // Auto-select the cutoff option at current depth
                    if (index === 0) {
                      const cutoffHash = hashContent(cutoffText);
                      const cutoffOptionId = `${node.id}-opt-${cutoffHash}`;
                      setSelectedMap((prev) => ({
                        ...prev,
                        [node.id]: cutoffOptionId,
                      }));
                    }
                  }
                }
              } catch (error) {
                console.error("Failed to generate rollout", { error, index });
              } finally {
                adjustGenerating(-1);
              }
            })()
      ...
      console.error("Failed to generate rollout", error);
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div className="space-y-2">
        <h1 className="text-3xl font-semibold">Rollouts</h1>
        <p className="text-muted-foreground max-w-3xl">
          Generate multiple continuations from any point in a conversation. Select text in any response,
          then click Rollout to explore different ways the conversation could continue from that exact point.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Select
          ...
          onValueChange={(value) => setSelectedExample(value as keyof typeof exampleSets)}
        >
          ...
          <SelectContent>
            ...
            <SelectItem value="buildComputer">Build Computer</SelectItem>
          ...
          return path.map((step, index) => {
            ...
            return (
              <ConversationNodeCard
                ...
                onRollout={(node, option, cutoffText) =>
                  handleRollout(
                    node,
                    option,
                    cutoffText,
...
interface ConversationNodeCardProps {
  ...
  onTextSelection: (nodeId: string, fullText: string) => void;
  onEditNode: (
    node: ConversationNode,
    selectedOption: ConversationOption | null,
    runId: string,
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex flex-wrap items-center gap-2">
            {node.role === "assistant" && selectedOption && (
              <>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => {
                    const textToUse = selectionNodeId === node.id && selectedText
                      ? selectedText
                      : selectedOption.content;
                    onRollout(node, selectedOption, textToUse);
                  }}
                  disabled={isGenerating}
                >
                  {isGenerating ? "Generating…" : "Rollout"}
                </Button>
                {selectionNodeId === node.id && selectedText && (
                  <span className="text-xs text-muted-foreground">
          ...
          {hasOptions && selectedOption ? (
            <div className="space-y-3">
              <p
                ...
                onMouseUp={() => onTextSelection(node.id, selectedOption.content)}
